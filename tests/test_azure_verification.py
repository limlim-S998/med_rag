...
import pytest
from integration.azure_checks import (
...
def collector(tmp_path):
    ...
    value.resources = {}
    value.kubeconfig = tmp_path / "kubeconfig"
    value.study, value.section = "study", "section"
...
def test_all_release_revisions_must_settle_before_reading_pod_identities(tmp_path, monkeypatch):
    ...
    def kube(*args, **kwargs):
        ...
        if "helmreleases" in args:
            ...
            return {"items": [{"metadata": {"name": name, "generation": 2},
                ...
                for name in (*SERVICES, "qdrant", "airflow")]}
        if "exec" in args:
            assert snapshots == 2
    ...
    value.kube = kube
    monkeypatch.setattr("integration.azure_checks.time.sleep", lambda _: None)
...
def test_persistence_requires_recovered_public_search_and_retained_source(tmp_path, monkeypatch, scenario):
    ...
    monkeypatch.setattr("integration.azure_checks.httpx.Client",
                        lambda **kwargs: real_client(transport=httpx.MockTransport(handle), **kwargs))
    ticks = iter(range(0, 1000, 10))
    monkeypatch.setattr("integration.azure_checks.time.monotonic", lambda: next(ticks))
    monkeypatch.setattr("integration.azure_checks.time.sleep", lambda _: None)
    if scenario == "recover":
...
from integration.azure_checks import INFLIGHT_METRIC
...
def test_scaling_waits_for_scrapes_and_rejects_incomplete_drafts(tmp_path, monkeypatch, scenario):
    ...
    value = collector(tmp_path)
    value.base, value.tls, value.token = "https://api.invalid", True, "PRIVATE-TOKEN"
    value.study, value.section = "S1", "section"
    value.completed_workflows = [{"source_revision": "retained"}]
    ...
    monkeypatch.setattr("integration.azure_checks.httpx.Client", client)
...
def test_snapshot_restore_is_isolated_and_cleanup_runs_on_failure(tmp_path, monkeypatch, capsys, fail_restore):
    ...
    value.kube = kube
    value.apply = lambda *objects: applied.extend(objects)
...
def test_negative_api_checks_keep_jwt_out_of_blob_capability_request(tmp_path, monkeypatch):
    ...
    value = collector(tmp_path)
    value.base, value.tls, value.token = "https://api.invalid", True, "API-TOKEN"
    value.study, value.section = "S1", "11.2"
    requests = []
