...
def test_search_request_excludes_documentation_but_keeps_generation_contract():
    request = json.loads((ROOT / "infra/search/csr-chunks-index.json").read_text())
...
def test_migration_shell_parses_without_running_azure():
    subprocess.run(["bash", "-n", str(ROOT / "scripts/migrate.sh")], check=True)
