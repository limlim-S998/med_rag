...
resource "azurerm_application_insights" "application" {
  ...
  tags                 = local.tags
  # Create the expected action group first so Azure reuses our managed group.
  # On destruction the component goes before the group, preventing recreation.
  depends_on = [azurerm_monitor_action_group.smart_detection]
}

# Azure creates these defaults alongside Application Insights, but deleting the
# component alone leaves them behind. Own the exact defaults in this dedicated
# application resource group; never claim monitoring resources in shared groups.
...
resource "azurerm_monitor_smart_detector_alert_rule" "failure_anomalies" {
  # Azure can create this rule before Terraform reaches it. The isolated alias
  # adopts that exact generated rule into state instead of requiring a separate
  # import after every fresh deployment. It must not be used for other resources.
  provider            = azurerm.generated_monitoring
  name                = "Failure Anomalies - ${azurerm_application_insights.application.name}"
  resource_group_name = local.rg
  scope_resource_ids  = [azurerm_application_insights.application.id]
